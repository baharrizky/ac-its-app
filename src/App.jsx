...
const MATERI = {
  ...
  E11: {
    formula: [
      ...
      "F(x)^{h(x)} = g(x)^{h(x)} \\;\\Rightarrow\\; F(x)=g(x)\\ \\text{ATAU}\\ h(x)=0\\ \\text{ATAU}\\ F(x)=-g(x)\\ (h(x)\\text{ genap})",
      "P\\cdot(a^{x})^{2} + Q\\cdot a^{x} + R = 0 \\;\\xrightarrow{u=a^{x}}\\; Pu^{2}+Qu+R=0",
    ],
    penjelasan: "Kalau basis kedua ruas SUDAH sama (atau bisa disamakan dengan sifat eksponen), langsung samakan pangkatnya: $f(x)=g(x)$. Kalau basisnya berbeda (berupa bilangan konstan $a,b$) tapi pangkatnya sudah sama persis di kedua ruas, satu-satunya cara persamaan itu benar adalah pangkatnya $=0$ (karena $a^0=b^0=1$). Kalau basis KEDUA ruas berupa fungsi $F(x)$ dan $g(x)$ tapi pangkatnya sama, $h(x)$, ada TIGA kemungkinan supaya persamaan itu benar: (a) $F(x)=g(x)$ — basisnya sama persis; (b) $h(x)=0$, dengan syarat $F(x)$ dan $g(x)$ tidak boleh $0$ (karena bilangan$^0=1$); (c) $F(x)=-g(x)$, tapi HANYA berlaku kalau $h(x)$ genap (karena basis positif dan negatif yang berlawanan tanda akan sama kalau dipangkatkan genap, misalnya $1^2=(-1)^2$) — kalau $h(x)$ ternyata ganjil/pecahan, solusi dari cara ini harus dibuang. Untuk bentuk yang menyerupai persamaan kuadrat (ada $a^{2x}$ dan $a^x$ dalam satu persamaan), misalkan $u=a^x$ dulu supaya jadi $Pu^2+Qu+R=0$, selesaikan $u$ dengan pemfaktoran/rumus ABC, baru kembalikan $a^x=u$ untuk mencari $x$ (nilai $u$ harus positif karena $a^x>0$).",
    contoh: [
      "3^{5x-9}=3^{2x+3} \\;\\to\\; 5x-9=2x+3 \\;\\to\\; 3x=12 \\;\\to\\; x=4",
      "8^{x+2}=32^{x-1} \\;\\to\\; (2^3)^{x+2}=(2^5)^{x-1} \\;\\to\\; 3x+6=5x-5 \\;\\to\\; x=\\tfrac{11}{2}",
      "\\begin{gathered}\\footnotesize\\text{Basis fungsi, pangkat sama: }(x-2)^{x+3}=(2x-5)^{x+3}\\\\[6pt] \\text{(a) }x-2=2x-5\\to x=3\\\\ \\text{(b) }x+3=0\\to x=-3\\ (F,g\\neq0,\\text{valid})\\\\ \\text{(c) }x-2=-(2x-5)\\to x=\\tfrac73\\ (h(x)=\\tfrac{16}{3}\\text{ tak genap, ditolak})\\\\ \\Rightarrow HP=\\{-3,\\,3\\}\\end{gathered}",
...
const HINTS = {
  ...
  E11: { t1: "Kalau basis kedua ruas sudah sama (atau bisa disamakan), langsung samakan pangkatnya: $f(x)=g(x)$. Kalau basisnya berupa FUNGSI ($F(x)$ dan $g(x)$) dengan pangkat yang sama, ada 3 kemungkinan, bukan cuma satu.", t2: "Kalau ada $a^{2x}$ DAN $a^x$ dalam satu persamaan, itu tandanya bentuk kuadrat tersamar — misalkan $u=a^x$ dulu, selesaikan $u$, baru cari $x$. Kalau bentuknya $F(x)^{h(x)}=g(x)^{h(x)}$, cek juga kemungkinan $h(x)=0$ dan $F(x)=-g(x)$ (khusus untuk $h(x)$ genap), jangan cuma menyamakan $F(x)=g(x)$.", full: "Basis konstan sama → samakan pangkat: $a^{f(x)}=a^{g(x)}\\Rightarrow f(x)=g(x)$. Basis konstan beda tapi pangkat sama di kedua ruas → pangkatnya harus 0. Basis berupa FUNGSI, $F(x)^{h(x)}=g(x)^{h(x)}$ → cek TIGA kemungkinan: (a) $F(x)=g(x)$; (b) $h(x)=0$ dengan syarat $F(x),g(x)\\neq0$; (c) $F(x)=-g(x)$ dengan syarat $h(x)$ genap (kalau $h(x)$ ganjil/pecahan, solusi ini dibuang). Bentuk kuadrat tersamar → substitusi $u=a^x$, selesaikan $Pu^2+Qu+R=0$, lalu kembalikan ke $a^x=u$ (ingat $u$ harus $>0$)." },
};
const PRACTICE_POOL = {
  E1: [
